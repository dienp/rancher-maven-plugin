...
public class RancherAPIConnectionMojo extends AbstractMojo {
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    String projectId =
        rancherApi.findProjectIdByName(rancherHost, rancherPort, rancherProjectName, authToken);
    String stackId = rancherApi.findStackIdByName(rancherHost, rancherPort, projectId,
        rancherStackName, authToken);
    ...
    if (serviceId.equals(Constant.SERVICE_ID_NOT_EXISTED)) {
      String createServiceURL = new URIBuilder(rancherHost, rancherPort, projectId, null)
          .buildCreateServiceURI().toString();
      ...
      String state = Util.findKeyValue(
          Util.fetchServiceInfoFromRancher(serviceWithIdURL, authToken), Constant.KEYWORD_STATE);

      switch (state) {
        case Constant.State.ACTIVATING:
        case Constant.State.ACTIVE:
          upgradeService(upgradeURL, rollbackURL, finishUpgradeURL, serviceWithIdURL,
              dockerImageName, actionTimeout, authToken, launchConfig);
          break;
        case Constant.State.UPGRADED:
          finishUpgradeService(finishUpgradeURL, authToken);
          if (Util.pollingForState(serviceWithIdURL, authToken, actionTimeout,
  ...
  private void upgradeService(String upgradeURL, String rollbackURL, String finishUpgradeURL,
      String serviceWithIdURL, String dockerImage, Long actionTimeout, String authToken,
      JsonNode launchConfig) {
    LOGGER.debug("Upgrading service...");
    Util.postToRancher(upgradeURL, Util.makeUpgradeServicePostData(dockerImage, launchConfig),
        authToken);
  ...
  private void createService(String createServiceURL, String name, String stackId,
      String dockerImage, String authToken) {
    LOGGER.debug("Creating service...");
    Util.postToRancher(createServiceURL, Util.makeCreateServicePostData(name, stackId, dockerImage),
        authToken);
  }
